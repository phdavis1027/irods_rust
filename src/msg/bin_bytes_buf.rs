        ...
        's: 'r,
    {
        ...
        loop {
            state = match (state, reader.read_event()?) {
                ...
                (State::Buf, Event::Start(ref e)) if e.name().as_ref() == b"buf" => {
                    State::BufInner
                },
                (State::BufInner, Event::Text(ref e)) => {
                    return Ok(BorrowingStrBuf {
                        buf: e.unescape()?,
                    });
                },
                (state, Event::Eof) => return Err(IrodsError::Other(format!("unexpected EOF in state: [{:?}]", state))),
                (state, _) => state,
            };
        }
...
mod tests {
    ...
    fn test_borrowing_str_buf_serializes_correctly() {
        let expected = r#"<BinBytesBuf_PI><bufLen>11</bufLen><buf>hello world</buf></BinBytesBuf_PI>"#;
